...
int main(void)
{
   ...
   // ToggleFullscreen();
    ...
    //background sound------------------------------------------------------------------------
    Music backmusic = LoadMusicStream("D:\\ROCKET GAME\\ROCKET GAME\\ROCKET GAME\\backsound.mp3");
    ...
    // ---------------------------movement in x-axis and y-axis for my rocket blue _ white------------------------------------
    ...
    for (int i = 0; i < Enemy_Amount; i++) {
        enemy[i].enemyplanepos = (Vector2){ enemyX,enemyY };  //position of enemy plane every time it respwan but it is used in other way mens when W is pressed
    ...
    while (!WindowShouldClose()) {
        ...
        if (counter == 2) {
            ...
            DrawText("WELL DONE LEGEND NOW PRESS\nW TO GAIN MORE PRIDE ",650,730,40,PURPLE);
            //sending planes to their origin:
            rocketposition.x = 650.0f;
            rocketposition.y = 750.0f;
            for (int i = 0; i < Enemy_Amount; i++) {
                enemy[i].enemyplanepos = (Vector2){ enemyX,enemyY };  //position of enemy plane every time it respwan but it is used in other way means below line read plz
                enemy[i].enemyframerec.x = 290.0f;
                enemy[i].enemyframerec.y = 0.0f;
                enemy[i].active = true;
                enemy[i].enemyframerec.width = 100;                      //here we are again sending enemy to its orignal position
                enemy[i].enemyframerec.height = 152;
            }//loop for enemy
        ...
        //actually in the above result loop we have changes the planes position and again call our game:
            ...
            //----------------------------------key controls------------------------------------------------------------------------
                ...
                if (movementbool) {// actually this controls the movement of player untill it does not collied with enemy
                    if (IsKeyDown(KEY_RIGHT)) {
                        ...
                        if (rocketposition.x + 100 >= screenWidth) rocketposition.x = screenWidth - 100;
                    }
                    ...
                    if (IsKeyDown(KEY_DOWN)) {
                        rocketposition.y += movey;
                        if (rocketposition.y + 100 >= screenHeight) rocketposition.y = screenHeight - 100;
                    }
                }//end of movement if
                //-------------------------key control end----------------------------------------------------------------------------
                    ...
                    if (bullets[m].active) {
                        for (int a = 0; a < Enemy_Amount; a++) {
                            if (enemy[a].active && CheckCollisionCircleRec(bullets[m].bulletposition, bullets[m].radius, enemy[a].enemycollision)) {
                                ...
                                //callenemy(enemy[Enemy_Amount+1]);
